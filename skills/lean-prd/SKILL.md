...
```
## Problem
[1 to 2 paragraphs describing the problem being solved. Who is affected, what is the pain, and why does it matter now?]

## Solution
[1 to 2 paragraphs describing the proposed solution at a high level. What are we building, and how does it address the problem?]
...
- Write 4 to 8 top-level stories in the format: `As a [user], I want [goal] so that [benefit]`
...
Before writing to Notion, show the user the full draft in chat (properties plus body) and ask whether to save. Show it in this format:
...
When writing copy for the user (PRD body, Open Questions, anything that ends up in Notion or chat), follow these rules:
...
These rules apply to the PRD body that gets saved to Notion AND to the chat copy shown during the review step. If the user provides input that contains em-dashes, preserve their wording when quoting them directly but don't introduce new ones.
