...
2. **Linear-ready tickets**: a parent issue (acts as the epic) plus child issues under the **Just-fix** team
...
- Break each activity into 2 to 5 tasks. These become individual Linear child issues
...
**Description:** [1 to 2 sentences expanding on the story if needed]
**Acceptance Criteria:**
...
- Aim for 2 to 4 criteria per story. Enough to verify done, not a full spec
...
**Description:** [2 to 3 sentences summarising what this epic delivers and why. Reference the Notion PRD URL.]
...
When writing copy for the user (story summaries, acceptance criteria, parent issue descriptions, anything that ends up in Linear or chat), follow these rules:
...
These rules apply to the ticket content that gets written to Linear AND to the chat copy shown during the preview step. If the user provides input that contains em-dashes, preserve their wording when quoting them directly but don't introduce new ones.
